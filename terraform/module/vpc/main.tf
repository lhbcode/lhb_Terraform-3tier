...
resource "aws_subnet" "public" {
  ...
  tags = merge(
    {
      "Name" = format(
        "%s%s-${var.public_subnet_suffix}-sub",
        var.sub_name,
        substr(element(var.azs, count.index),14,1),
      )
    },
    var.tags,
...
resource "aws_subnet" "private" {
  ...
  tags = merge(
    {
      "Name" = format(
        "%s%s-${var.private_subnet_suffix}-sub",
       var.sub_name,
        substr(element(var.azs, count.index),14,1),
      )
    },
...
resource "aws_subnet" "database" {
  ...
  tags = merge(
    {
      "Name" = format(
        "%s%s-${var.database_subnet_suffix}-sub",
        var.sub_name,
        substr(element(var.azs, count.index),14,1),
      )
    },
  ...
  tags = merge(
    {
      "Name" = format(
        "%s%s-natgw",
        var.sub_name,
        substr(element(var.azs, var.single_nat_gateway ? 0 : count.index),14,1),
      )
    },
    var.tags,
